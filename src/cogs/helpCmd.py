...
from utils.misc import split_text

log = logging.getLogger(__name__)

support_link = "https://discord.gg/3Ugade9"

help_embed_color = 0x000000
...
class EmbedHelp(dpy_cmds.DefaultHelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        embed = discord.Embed(title='`void` Help', color=help_embed_color)

        embed.add_field(name="What is `void` for?",
                        value="`void` is a bot for keeping discord channels clear of messages. It's original purpose is for `screaming into the void` channels, but use it for what ever, I don't care.")

        embed.set_footer(text="Created by Luna, Hibiki, and Fluttershy aka Amadea System (Hibiki#8792) "
                              "| Github: https://github.com/amadea-system/void/")
    ...
    async def send_command_help(self, command: dpy_cmds.Command):
        ...
        embed = discord.Embed(title=f'{command_name}  -  `void` Help', color=help_embed_color)
        embed.description = "\n".join(embed_descrip)
